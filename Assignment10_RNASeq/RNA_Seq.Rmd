...
```{r}
BiocManager::install('openssl-devel')
BiocManager::install('curl')
BiocManager::install('curl-devel')
```
...
```{r cache=TRUE}
getwd()
d <- read.table("pnas_expression.txt",header=T)
...
d.pca$x
biplot(d.pca)
plot(d.pca$x[,1], d.pca$x[,2], main="PCA",xlab="PC1",ylab="PCA2")
...
```{r}
# changing rvchack version to then install clusterProfiler
